    ...
    <div class="space-y-3 p-4">
      <div class="flex items-center justify-between border-b border-gray-100 pb-2">
        ...
        <span
          ...
          class:bg-green-100={robotResult?.health === 'healthy'}
          class:text-green-700={robotResult?.health === 'healthy'}
          class:bg-yellow-100={robotResult?.health === 'warning'}
          class:text-yellow-700={robotResult?.health === 'warning'}
          class:bg-red-100={robotResult?.health === 'critical'}
          class:text-red-700={robotResult?.health === 'critical'}
        >
          {getStatusText(robotResult?.health)}
        </span>
      </div>
      <div>
        <strong class="mb-1 block text-xs text-gray-500">診断</strong>
        <p class="text-sm text-gray-800">{robotResult?.diagnosis}</p>
      </div>
      <div class="border-t border-gray-100 pt-2">
        <strong class="mb-1 block text-xs text-gray-500">推奨アクション</strong>
        <p class="text-sm text-gray-800">{robotResult?.action}</p>
      </div>
    </div>
  </div>
</div>
